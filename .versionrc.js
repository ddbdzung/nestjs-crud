...
module.exports = {
  ...
  whatBump: (commits) => {
    ...
    for (const commit of commits) {
      ...
      const hasBreakingNote = commit.notes.some((n) => n.title === 'BREAKING CHANGE');
      ...
      if (hasBreakingNote || headerHasBang || isBreakingType) {
        ...
        return { 
          level: 0, 
          reason: 'breaking changes detected' + (commit.subject ? ` (${commit.subject})` : '')
        };
      }

      // Evaluate commit type for bump level
      const typeCfg = (module.exports.types || []).find((t) => t.type === commit.type);
